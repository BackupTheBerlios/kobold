\subsection{Kobold Serverdienst}
...
so werden in seinen Client die Daten der ihm unterstehenden Rollen und
zugehörigen Produktlinien bzw. Produkte geladen. Der Benutzer
...
Authentifizierungs-, Produktlinien- und Produktrelationen. Ihre Grundstruktur ist
baumartig, sie ist im Speicher auf dem aktuellen Stand und
wird innerhalb eines konfigurierbaren Timeouts persistiert
(standardmäßig alle 10 Minuten).\par
...
Der Kobold Serverdienst bildet für jeden Kobold-Benutzer eine Nachrichten-Queue. Werden Nachrichten oder 
Workflow-Objekte an einen Benutzer geschickt, so werden diese in der Nachrichten-Queue 
gespeichert und bei der nächsten Abfrage des Clients vom zugehörigen
Benutzer an diesen weitergeleitet.\par
Die vom Server angebotene Nachrichten-Queue ist zustandsbehaftet, das heißt bei
nicht-physisch begründeten Ausfällen ist die Nachrichten-Queue in der Regel ohne Datenverlust
...
\paragraph{Workflow-Mechanismus auf dem Kobold Server}
Sendet ein Client dem Server eine Nachricht, wendet dieser diese
Nachricht als Fakt auf eine kontextabhängige Regelmenge aus der Regelbasis an,
die sich auf dem Server befindet. Ein Fakt ist dabei eine Mitteilung an den Server. 
Es werden dabei alle Regeln der Regelmenge durchlaufen. Gilt eine
Bedingung einer Regel, so wird die zugehörige Konsequenz ausgeführt.
Im Implementierungsteil einer solchen Konsequenzen werden Modifikationen
an einem erzeugenden Workflow-Objekt durchgeführt (z.B. wird bei einem
unauthorisierten Commit ein String "Unauthorisierter Commit" in das
Workflow-Objekt geschrieben, sofern eine Regel dies definiert hat).
\par
