\chapter{Der Kobold Server}

Der Kobold Server koordiniert die Arbeit der Benutzer von Kobold. Ein Benutzer ist dabei ein Benutzername 
in Kobold. Diese sind dadurch bei ihrer Arbeit nicht an einen
festen Rechner gebunden, sondern können von jedem Rechner mit einem Kobold Client über den Kobold Server an 
ihre Daten gelangen und diese bearbeiten. Ausserdem ermöglicht
...
\subsection{Gespeicherte Daten auf dem Kobold Server}
...
Produktlinieningenieur für Produktlinie YZ. Diese Rollen werden in einer Liste unter dem Benutzername 
...
Ist das Element ein Produkt, so werden auch die Daten der dazugehörigen Produktlinie erfasst.\newline

Für ein Repository werden folgende Daten gespeichert:
\begin{itemize}
\item Pfad
\item Passwort
\item Benutzername
...
Datenhaltung flexibel zu organisieren.

\subsection{Nachrichten-Queue}

Der Kobold Server bildet für jeden Kobold-Benutzer eine Nachrichten-Queue. Werden Nachrichten oder Workflow-
Objekte an einen Benutzer geschickt, so werden dieser in der Nachrichten-Queue in Form einer Warteschlange 
...
Für Nachrichte auf der Nachrichten-Queue werden folgende Daten benötigt:
\begin{itemize}
\item Absender
\item Empfänger
\item Inhalt
