...
public class MetaInformation {
    ...
    public void createMetaInfoForPL (Document document, Productline pLine) {
        ...
        ite.hasNext();)
        {
        	User u1 = (User) ite.next();
        	createMetaInfoForMaintainer (document, u1);
        }
       		
        for (Iterator ite = pl.getProducts().iterator(); 
		ite.hasNext();)
        {
			Product p1 = (Product) ite.next();
			createMetaInfoForProduct (document, p1);
		}
    
		for (Iterator ite = pl.getComponents().iterator(); 
		ite.hasNext();)
		{
			Component c1 = (Component) ite.next();
			createMetaInfoForComponent (document, c1);
		}	
    ...
    public void createMetaInfoForProduct(Document document, Product p1) {
		...
		ite.hasNext();)
		{
			RelatedComponent rc1 = (RelatedComponent) ite.next();
			createMetaInfoForRelatedComponent(document, rc1);
		}
    	for (Iterator ite = product.getSpecificComponents().iterator(); 
		ite.hasNext();)
		{
    		SpecificComponent spc1 = (SpecificComponent) ite.next();
			createMetaInfoForSpecificComponents(document, spc1);
		}	
	
    }
    
    //MetaInformation for SpecificComponents	
	private void createMetaInfoForSpecificComponents(Document document, SpecificComponent spc1) 
	{
		
	}	
	private void createMetaInfoForComponent(Document document, Component c1)
	{
			
	}
	private void createMetaInfoForRelatedComponent(Document document, RelatedComponent rc1) 
	{
		
	}
	private void createMetaInfoForMaintainer(Document document, User u1) 
	{
